...
//
// 反幻觉约束：工具集是每轮动态注入的，本轮以系统提示词中注入的工具名为准。
// 用中性机制解释（而非指责"编造"）：之前轮的工具列表与本轮无直接关系，
// 可能一样也可能不一样——模型不必因本轮列表与之前不同而自我怀疑或否认，只需
// 如实依据本轮注入的工具回答即可。这样既能如实感知，又不陷入自我否定。
...
【工具列表是每轮动态变化的】：系统会在每一轮对话时重新注入本轮实际可用的工具列表
（见下方"当前可用工具"一行）。每一轮的列表都可能不同——用户可随时增删工具，本轮
与上一轮可能一样，也可能完全不同。因此：
1. 你只依据本轮注入的"当前可用工具"一行来判断本轮有哪些工具，这是唯一权威来源。
2. 之前轮次里出现过（或没有出现）的工具名，与本轮可用与否没有直接关系。不要因本轮
   列表与之前不同就自我怀疑或否认——这不代表你判断错了，只是工具集确实变了。
3. 若"当前可用工具"一行为空，说明本轮没有任何工具，你如实说明并仅凭自身知识回答即可；
   不要为了"保持工具印象"而谎称有工具，也不要伪造工具调用。`
...
// 空时注入"当前无工具"的明确声明（与静态段的反幻觉约束呼应）：避免模型靠历史
// 对话臆测旧工具、或伪称仍可调用。
func BuildSystemPromptWithMedia(base string, filesBaseURL string, toolNames ...string) string {
	...
	if len(toolNames) > 0 {
		toolUsage += "\n\n当前可用工具：" + strings.Join(toolNames, "、") +
			"。请据此判断该调用哪个工具完成任务。"
	} else if toolNames != nil {
		// 显式传入空列表（非 nil）：本会话当前没有任何可用工具。
		toolUsage += "\n\n当前可用工具：（空，本会话当前没有任何可调用工具）" +
			" 请如实告知用户你没有工具，并仅凭自身知识回答；不得伪造工具调用。"
	}
	if base == "" {
