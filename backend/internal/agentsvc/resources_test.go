...
func TestResourceToTools(t *testing.T) {
	cases := []struct {
		...
		{"能力翻译", []string{"search"}, []string{"web_search"}},
		{"能力去重", []string{"search", "file", "search"}, []string{"web_search", "file_ops"}},
...
func TestBuildResourceTools_PresenceSemantics(t *testing.T) {
	...
	t.Run("能力白名单+技能白名单", func(t *testing.T) {
		...
		want := []string{"web_search", "calculator", "skill_emoji-helper"}
		if !sameStringSet(tools, want) {
	...
	t.Run("仅能力白名单", func(t *testing.T) {
		...
		want := append([]string{"web_search"}, allSkills...)
		if !sameStringSet(tools, want) {
